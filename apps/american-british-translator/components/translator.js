...
class Translator {
  static replaceCurry(_, replacement, highlight, adjustCase = false) {
    if (adjustCase) {
      replacement = replacement.replace(/^([a-z])/gi, letter =>
        letter.toUpperCase()
      );
    }
    return () => {
      if (highlight) {
        return `<span class="highlight">${replacement}</span>`.replace(
          /\s/g,
          '\0'
        );
      } else {
  ...
  static translateAmericanToBritish(input, highlight = false) {
    ...
    input = input.replace(/^([a-z])/gi, letter => letter.toUpperCase());
